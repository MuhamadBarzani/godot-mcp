...
func _init() -> void:
	...
	# MeshLibrary authoring (issue #83) — build the library that GridMap cells reference.
	_handlers["cmd_create_mesh_library"] = _cmd_create_mesh_library
	_handlers["cmd_add_mesh_library_item"] = _cmd_add_mesh_library_item
	...
	# TileSet authoring (issue #82) — build the TileSet that tilemap cells reference.
	_handlers["cmd_create_tileset"] = _cmd_create_tileset
	_handlers["cmd_add_tileset_atlas_source"] = _cmd_add_tileset_atlas_source
	_handlers["cmd_create_tile"] = _cmd_create_tile
	...
	# Profiling (issue #38) — read-only Performance monitors (editor + running game).
	_handlers["cmd_get_editor_performance"] = _cmd_get_editor_performance
	_handlers["cmd_get_performance_monitors"] = _cmd_get_performance_monitors
	# Batch / refactor (issue #48) — operate over many nodes/scenes.
	_handlers["cmd_find_nodes_by_type"] = _cmd_find_nodes_by_type
	_handlers["cmd_batch_set_property"] = _cmd_batch_set_property
	_handlers["cmd_cross_scene_set_property"] = _cmd_cross_scene_set_property
	_handlers["cmd_get_dependencies"] = _cmd_get_dependencies
	...
	# Input map editing (issue #81) — edit project InputMap actions/events.
	_handlers["cmd_add_input_action"] = _cmd_add_input_action
	_handlers["cmd_remove_input_action"] = _cmd_remove_input_action
	_handlers["cmd_add_input_event"] = _cmd_add_input_event
	_handlers["cmd_clear_input_action_events"] = _cmd_clear_input_action_events
...
# --- MeshLibrary authoring (issue #83) -------------------------------------
# Builds the MeshLibrary (items = meshes by id) that gridmap_set_cell places. A
# MeshLibrary lives either on an in-scene GridMap (mutations are UndoRedo-wrapped)
# or in a saved .tres (mutations are re-saved via ResourceSaver). Mirrors the
# TileSet authoring handlers (#82).
...
func _cmd_create_mesh_library(params: Dictionary) -> Dictionary:
	...
	if node_path.is_empty() and save_path.is_empty():
		return _fail("VALIDATION_ERROR", "Provide 'node_path' to assign the MeshLibrary and/or 'save_path' to save it as a .tres.")
...
func _cmd_add_mesh_library_item(params: Dictionary) -> Dictionary:
	...
	if item_id < 0:
		return _fail("VALIDATION_ERROR", "'item_id' must be non-negative.")
	if library.get_item_list().has(item_id):
		return _fail("VALIDATION_ERROR", "Item id %d already exists in the MeshLibrary." % item_id)
...
## Resolve the target MeshLibrary from either an in-scene GridMap (`node_path`) or a
## saved .tres (`library_path`). Returns {ok, library, backing, node?|path?} or a
## structured failure. `backing` is "node" (UndoRedo) or "file" (re-save).
func _resolve_mesh_library(params: Dictionary) -> Dictionary:
	...
	if not node_path.is_empty() and not library_path.is_empty():
		return _fail("VALIDATION_ERROR", "Pass only one of 'node_path' or 'library_path', not both.")
	if not node_path.is_empty():
		var found := _resolve(node_path)
		if not found["ok"]:
			return found
		...
		if lib == null:
			return _fail("VALIDATION_ERROR", "GridMap has no mesh_library; create one first with create_mesh_library.", "mesh_library")
		return {"ok": true, "library": lib, "backing": "node", "node": node}
	if not library_path.is_empty():
		if not ResourceLoader.exists(library_path):
			return _fail("RESOURCE_NOT_FOUND", "No resource at '%s'." % library_path)
		var res: Resource = ResourceLoader.load(library_path)
		if not (res is MeshLibrary):
			return _fail("VALIDATION_ERROR", "'%s' is not a MeshLibrary." % library_path)
		return {"ok": true, "library": res, "backing": "file", "path": library_path}
	return _fail("VALIDATION_ERROR", "Provide a 'node_path' (GridMap) or a 'library_path' (.tres).")


## Resolve the item's Mesh from exactly one of `mesh_type` (a primitive, configured with
## `properties`) or `mesh_path` (an imported Mesh resource). Returns {ok, mesh} or a
## structured failure.
func _resolve_mesh(params: Dictionary) -> Dictionary:
	...
	if not mesh_type.is_empty() and not mesh_path.is_empty():
		return _fail("VALIDATION_ERROR", "Pass only one of 'mesh_type' or 'mesh_path', not both.")
	if not mesh_path.is_empty():
		if not ResourceLoader.exists(mesh_path):
			return _fail("RESOURCE_NOT_FOUND", "No resource at '%s'. Import the mesh into the project first." % mesh_path)
		var res: Resource = ResourceLoader.load(mesh_path)
		if not (res is Mesh):
			return _fail("VALIDATION_ERROR", "'%s' is not a Mesh resource (.glb/.gltf import as scenes, not meshes)." % mesh_path)
	...
	if not mesh_type.is_empty():
		...
		if not (obj is Mesh):
			if not (obj is RefCounted):
				obj.free()
			return _fail("VALIDATION_ERROR", "'%s' is not a Mesh." % mesh_type)
		_apply_props(obj, params.get("properties", {}))
		return {"ok": true, "mesh": obj}
	return _fail("VALIDATION_ERROR", "Provide 'mesh_type' (a primitive like BoxMesh) or 'mesh_path' (a Mesh resource).")


## Save a MeshLibrary resource to a .tres and tell the editor filesystem to refresh it.
func _save_mesh_library_file(library: MeshLibrary, path: String) -> Dictionary:
	var err := ResourceSaver.save(library, path)
	if err != OK:
		return _fail("INTERNAL_ERROR", "Failed to save MeshLibrary to '%s' (error %d)." % [path, err])
	EditorInterface.get_resource_filesystem().update_file(path)
	return {"ok": true}
...
# --- TileSet authoring (issue #82) -----------------------------------------
# Builds the TileSet (atlas source + tiles) that tilemap_set_cell/fill_rect place.
# A TileSet lives either on an in-scene TileMap/TileMapLayer (mutations are
# UndoRedo-wrapped) or in a saved .tres (mutations are re-saved via ResourceSaver).
...
func _cmd_create_tileset(params: Dictionary) -> Dictionary:
	...
	var save_path := str(params.get("save_path", ""))
	if node_path.is_empty() and save_path.is_empty():
		return _fail("VALIDATION_ERROR", "Provide 'node_path' to assign the TileSet and/or 'save_path' to save it as a .tres.")
	var size_res := _parse_vec2i(params.get("tile_size", [16, 16]), "tile_size")
	if not size_res["ok"]:
		return size_res
	var tile_size: Vector2i = size_res["value"]
	if tile_size.x <= 0 or tile_size.y <= 0:
		return _fail("VALIDATION_ERROR", "'tile_size' components must be positive.")
...
func _cmd_add_tileset_atlas_source(params: Dictionary) -> Dictionary:
	...
	if not (tex_res is Texture2D):
		return _fail("VALIDATION_ERROR", "'%s' is not a Texture2D." % texture_path)
	var region_res := _parse_vec2i(params.get("region_size", [16, 16]), "region_size")
	if not region_res["ok"]:
		return region_res
	var region: Vector2i = region_res["value"]
	if region.x <= 0 or region.y <= 0:
		return _fail("VALIDATION_ERROR", "'region_size' components must be positive.")
	var requested: Variant = params.get("source_id")
	var source_id := int(requested) if requested != null else tileset.get_next_source_id()
	if source_id < 0:
		return _fail("VALIDATION_ERROR", "'source_id' must be non-negative.")
	if tileset.has_source(source_id):
		return _fail("VALIDATION_ERROR", "Source id %d already exists in the TileSet." % source_id)
...
func _cmd_create_tile(params: Dictionary) -> Dictionary:
	...
	if not tileset.has_source(source_id):
		return _fail("VALIDATION_ERROR", "No source with id %d; add one with add_tileset_atlas_source." % source_id, "source_id")
	var source_obj: Object = tileset.get_source(source_id)
	if not (source_obj is TileSetAtlasSource):
		return _fail("VALIDATION_ERROR", "Source %d is not a TileSetAtlasSource." % source_id)
	var source: TileSetAtlasSource = source_obj
	var coords_res := _parse_vec2i(params.get("atlas_coords"), "atlas_coords")
	if not coords_res["ok"]:
		return coords_res
	var coords: Vector2i = coords_res["value"]
	var size_res := _parse_vec2i(params.get("size", [1, 1]), "size")
	if not size_res["ok"]:
		return size_res
	var size: Vector2i = size_res["value"]
	if size.x <= 0 or size.y <= 0:
		return _fail("VALIDATION_ERROR", "'size' components must be positive.")
	...
	if not source.has_room_for_tile(coords, size, 1, Vector2i.ZERO, 1):
		return _fail("VALIDATION_ERROR", "No room for a tile at %v size %v (out of the atlas grid or overlapping an existing tile)." % [coords, size])
	if resolved["backing"] == "node":
		var ur := EditorInterface.get_editor_undo_redo()
...
## Resolve the target TileSet from either an in-scene TileMap/TileMapLayer node
## (`node_path`) or a saved .tres (`tileset_path`). Returns {ok, tileset, backing,
## node?|path?} or a structured failure. `backing` is "node" (UndoRedo) or "file"
## (re-save via ResourceSaver).
func _resolve_tileset(params: Dictionary) -> Dictionary:
	...
	if not node_path.is_empty():
		...
		if ts == null:
			return _fail("VALIDATION_ERROR", "Node has no tile_set; create one first with create_tileset.", "tile_set")
		return {"ok": true, "tileset": ts, "backing": "node", "node": node}
	if not tileset_path.is_empty():
		if not ResourceLoader.exists(tileset_path):
			return _fail("RESOURCE_NOT_FOUND", "No resource at '%s'." % tileset_path)
		var res: Resource = ResourceLoader.load(tileset_path)
		if not (res is TileSet):
			return _fail("VALIDATION_ERROR", "'%s' is not a TileSet." % tileset_path)
		return {"ok": true, "tileset": res, "backing": "file", "path": tileset_path}
	return _fail("VALIDATION_ERROR", "Provide a 'node_path' (TileMap/TileMapLayer) or a 'tileset_path' (.tres).")


## Save a TileSet resource to a .tres and tell the editor filesystem to refresh it.
func _save_tileset_file(tileset: TileSet, path: String) -> Dictionary:
	var err := ResourceSaver.save(tileset, path)
	if err != OK:
		return _fail("INTERNAL_ERROR", "Failed to save TileSet to '%s' (error %d)." % [path, err])
	EditorInterface.get_resource_filesystem().update_file(path)
	return {"ok": true}
...
func _cmd_get_performance_monitors(_params: Dictionary) -> Dictionary:
	...
	_debugger.send_to_probe("godot_mcp:get_performance", [])
	var cached: Variant = _debugger.get_performance()
	return _ok({
		"playing": true,
		"connected": true,
		"ready": cached != null,
...
func _cmd_find_nodes_by_type(params: Dictionary) -> Dictionary:
	...
	if type_name.is_empty():
		return _fail("VALIDATION_ERROR", "'type' must be a non-empty class name.")
	...
	return _ok({"type": type_name, "nodes": nodes, "count": nodes.size()})
...
func _cmd_batch_set_property(params: Dictionary) -> Dictionary:
	...
	if property.is_empty():
		return _fail("VALIDATION_ERROR", "'property' must be a non-empty string.")
...
func _cmd_cross_scene_set_property(params: Dictionary) -> Dictionary:
	...
	if not (scenes is Array) or (scenes as Array).is_empty():
		return _fail("VALIDATION_ERROR", "'scenes' must be a non-empty array of res:// scene paths.")
	var type_name := str(params.get("type", ""))
	var property := str(params.get("property", ""))
	if type_name.is_empty() or property.is_empty():
		return _fail("VALIDATION_ERROR", "'type' and 'property' are required.")
	var value: Variant = params.get("value")
	var dry_run := bool(params.get("dry_run", false))
	...
	return _ok({"results": results, "total_modified": total, "scenes": results.size(), "dry_run": dry_run})
...
func _cmd_get_dependencies(params: Dictionary) -> Dictionary:
	...
	return _ok({"path": path, "dependencies": out, "count": out.size()})
...
func _batch_targets(root: Node, params: Dictionary) -> Variant:
	...
	if node_paths is Array and not (node_paths as Array).is_empty():
		...
		for raw in node_paths:
			...
			if node == null:
				return _fail("RESOURCE_NOT_FOUND", "No node at '%s'." % str(raw))
			nodes.append(node)
		return nodes
	if not type_name.is_empty():
		...
		return matches
	return _fail("VALIDATION_ERROR", "Provide 'node_paths' or 'type' to select targets.")
...
## Read export_presets.cfg with Godot's own ConfigFile (robust against its typed values).
## Returns {has_config, presets:[{index, name, platform, runnable, export_path}]}.
...
func _cmd_remove_input_action(params: Dictionary) -> Dictionary:
	...
	if action_name.is_empty():
		return _fail("VALIDATION_ERROR", "name is required.")
	if not params.get("confirm", false):
		return _fail("PRECONDITION_FAILED", "This call removes an action and all its events. Set confirm=True to proceed.", "confirm")
	var setting_key := "input/" + action_name
...
func _cmd_add_input_event(params: Dictionary) -> Dictionary:
	...
	if action_name.is_empty():
		return _fail("VALIDATION_ERROR", "action is required.")
	var setting_key := "input/" + action_name
	if not ProjectSettings.has_setting(setting_key):
		return _fail("RESOURCE_NOT_FOUND", "No input action '%s'." % action_name)
	...
	match event_type:
		"key":
			...
			if not code_str.is_empty():
				...
				return _fail("VALIDATION_ERROR", "Set 'keycode' or 'physical_keycode' for key events.")
		...
		"mouse":
			...
			match btn:
				...
				_:
					return _fail("VALIDATION_ERROR", "Unknown mouse button '%s'." % btn)
		"joypad_button":
		...
		_:
			return _fail("VALIDATION_ERROR", "Unknown event_type '%s'. Use key/mouse/joypad_button/joypad_motion." % event_type)
	if ev == null:
		return _fail("INTERNAL_ERROR", "Failed to build input event.")
	events.append(ev)
...
func _cmd_clear_input_action_events(params: Dictionary) -> Dictionary:
	...
	if action_name.is_empty():
		return _fail("VALIDATION_ERROR", "action is required.")
	if not params.get("confirm", false):
		return _fail("PRECONDITION_FAILED", "This call removes all events for '%s'. Set confirm=True to proceed." % action_name, "confirm")
	var setting_key := "input/" + action_name
	if not ProjectSettings.has_setting(setting_key):
		return _fail("RESOURCE_NOT_FOUND", "No input action '%s'." % action_name)
	var action_data: Dictionary = ProjectSettings.get_setting(setting_key)
	...
	if InputMap.has_action(action_name):
		for ev in InputMap.action_get_events(action_name):
			InputMap.action_erase_event(action_name, ev)
	return _ok({"action": action_name, "cleared": true})
